...
//--------------------------
    ...
    inline void SetUniform(int programId, const char* uniformName) const {
...
public:
    ...
    void SetUniform(int programId, const char* uniformName) const {
        int location = glGetUniformLocation(programId, uniformName);
        if (location >= 0) glUniformMatrix4fv(location, 1, GL_FALSE, (const GLfloat*)this);
        else printf("Uniform %s cannot be set\n", uniformName);
    }
